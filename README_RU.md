...
3.  **Правила доступа для ролей (Role Access Rules):**  правила связывающие **Роль** с **Таблицей**.
...
Права проверяются с использованием класса `CheckAccess`, который вызывается как зависимость.

**Примечание:** Зависимость `CheckAccess` вынесена в общую директорию `common/`. Это архитектурное решение позволяет использовать одну и ту же логику авторизации в нескольких микросервисах по мере масштабирования системы.
